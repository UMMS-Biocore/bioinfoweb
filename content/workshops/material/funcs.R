...
overlaySig <- function (avgall, res_selected)
{
...
plotScatter <- function(avgall, x, y, color="black")
{
  gdat<-data.frame(avgall)
  ggplot() +
    geom_point(data=gdat, aes_string(x=x, y=y),
               color=color, alpha=6/10, size=3) +
    scale_x_log10() +scale_y_log10()
}
plotOverlaidData <-function(overlaid_data, text_data, x="Treat", y="Control", label="rownames(text_data)")
{
  ggplot() +
    geom_point(data=overlaid_data, aes_string(x=x, y=y,
                                              colour="Legend"), alpha=6/10, size=3) +
    scale_colour_manual(values=c("All"="darkgrey","Significant"="red"))+
    geom_point(data=text_data, aes_string(x=x, y=y,label=label), colour="black", size=4)+
    geom_text(data=text_data, aes_string(x=x, y=y, label=label), colour="black", size=6, vjust=0, hjust=-0.1)+
    scale_x_log10() +scale_y_log10()
}
...
plotHeatmap<-function(method, cld )
{
  if (method=="euclidean")
  {
